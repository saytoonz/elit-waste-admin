...
@role('Owner|Admin')
<li>
    <a href="{{ route('sms_broadcasts.index') }}" class="{{ request()->routeIs('sms_broadcasts.*') ? 'bg-secondary text-white' : 'text-indigo-200 hover:text-white hover:bg-secondary' }} group flex gap-x-3 rounded-md p-2 text-sm font-semibold leading-6">
        <svg class="h-6 w-6 shrink-0 {{ request()->routeIs('sms_broadcasts.*') ? 'text-white' : 'text-indigo-200 group-hover:text-white' }}" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
...
<li>
    @php $paymentsOff = !\App\Support\PlatformConfig::paymentsEnabled(); @endphp
    <a href="{{ route('platform.settings.show') }}" class="{{ request()->routeIs('platform.settings.*') ? 'bg-secondary text-white' : 'text-indigo-200 hover:text-white hover:bg-secondary' }} group flex gap-x-3 rounded-md p-2 text-sm font-semibold leading-6">
        <svg class="h-6 w-6 shrink-0 {{ request()->routeIs('platform.settings.*') ? 'text-white' : 'text-indigo-200 group-hover:text-white' }}" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
        ...
        <span class="flex-1">Platform Settings</span>
        @if($paymentsOff)<span class="inline-flex items-center rounded-full bg-amber-300 px-1.5 py-0.5 text-[10px] font-bold text-amber-900">Paused</span>@endif
    </a>
</li>
@endhasrole
